...
export function StudiosSection() {
  ...
  return (
    <section
      id="produit"
      className="scroll-mt-24 border-y border-white/[0.06] bg-[#121212] px-5 py-20 sm:px-8 sm:py-28 lg:px-10 lg:py-36"
    >
        ...
        >
          {studioContent.map((studio, index) => (
            <button
              ...
              className={`rounded-full px-4 py-2 transition focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#1f8a65] ${active === index ? "bg-white text-[#121212]" : ""}`}
            >
              {studio.label}
            </button>
          ))}
                    ...
                    <div className="pointer-events-none absolute inset-x-4 bottom-4 flex flex-wrap gap-2">
                      {(studio.label === "Workout Studio"
                        ...
                        : [
                            ...
                            "Jours training / repos",
                            "Macros contextualisées",
                          ]
                      ).map((item, index) => (
                        <motion.span
                          key={item}
                          ...
                          className="rounded-full border-[0.3px] border-white/[0.06] bg-[#181818]/90 px-3 py-1.5 font-barlow-condensed text-[10px] uppercase tracking-[.12em] text-white/78 backdrop-blur-md"
                        >
                          {item}
                        </motion.span>
                      ))}
