...
function PrepCard({ prep, onLogged, onDeleted, onEdit, onToggleActive, slotLabels }: {
  ...
  slotLabels: Record<SmartPrepSlot, string>
}) {
  const { t } = useClientT()
    ...
    >
      ...
      >
        ...
        <div className="text-right shrink-0">
          ...
          <p className="text-[9px] uppercase tracking-[0.12em] text-white/25">{prep.is_active ? t('prep.applied') : t('prep.standbyLabel')}</p>
        </div>
      ...
      <AnimatePresence initial={false}>
        {expanded && (
          ...
          >
            ...
            {errorMsg && (
              <div className="mx-4 mb-2 rounded-xl bg-red-500/10 border border-red-500/15 px-3 py-2">
            ...
            <div className="px-4 pb-4 flex gap-2">
              ...
              >
                {busy === "toggle" ? "..." : prep.is_active ? t('prep.active') : t('prep.activate')}
              </button>
              <button
                onClick={logPrep}
                disabled={busy !== null}
    ...
    <section className="space-y-2">
      ...
      {!compact && (
        <div className="flex items-center justify-between px-1">
          <p className="text-[9px] font-bold uppercase tracking-[0.18em] text-white/25">{t('prep.mealsLabel')}</p>
          <p className="text-[10px] text-white/30">{t('prep.pending', { n: visiblePreps.length })}</p>
        </div>
      )}
          ...
          <div key={slot} className="space-y-2">
            <div className="flex items-center justify-between px-1">
              ...
              <p className="text-[10px] text-white/25">
                {t('prep.activeCount', { n: grouped[slot].filter(prep => prep.is_active).length })}
              </p>
            </div>
