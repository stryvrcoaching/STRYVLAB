...
import { TRAINING_ACCENT, VOLUME_OVERFLOW_COLOR } from '@/lib/nutrition/ui-colors'
...
type WindowData = { range_start: string; sessions_count: number; groups: Group[] }
...
export default function VolumeCoverageWidget({
  ...
  const FILTERS: Array<{ key: WindowKey; label: string }> = [
    { key: 'current_week', label: 'Semaine' },
    { key: '7d', label: '7j' },
    { key: '14d', label: '14j' },
    { key: '30d', label: '30j' },
  ]

  const emptyLabel = windowKey === 'current_week'
    ? t('msg.no.series.completed')
    : `Aucune série complétée sur ${FILTERS.find(f => f.key === windowKey)?.label.toLowerCase()}`
    ...
    <div className="bg-[#111111] rounded-2xl p-4">
      <div className="flex items-baseline justify-between mb-3">
        ...
        <span className="text-[10px] text-white/40 tabular-nums">
          {activeWindow.sessions_count > 0 ? `${activeWindow.sessions_count} séance${activeWindow.sessions_count > 1 ? 's' : ''}` : t('msg.no.session')}
        </span>
      </div>
      ...
      <div className="space-y-2.5">
        {activeGroups
          ...
          else if (g.actual > (g.mav || Infinity)) color = VOLUME_OVERFLOW_COLOR
