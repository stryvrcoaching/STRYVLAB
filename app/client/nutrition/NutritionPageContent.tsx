...
export type NutritionSearchParams = { date?: string; tab?: 'suivi' | 'planning' | 'tendances' }
...
export default async function ClientNutritionPage({ searchParams, mode = 'suivi' }: ClientNutritionPageProps) {
  const { date: dateParam, tab: tabParam } = await searchParams
  noStore()
  const activeTab = mode === 'plan' || tabParam === 'planning'
    ? 'plan'
    : tabParam === 'tendances'
      ? 'tendances'
      : 'suivi'
    ...
    svc()
      ...
      .select(`
        ...
        nutrition_entries (
          ...
          food_items (
            id, name_fr, category_l1, category_l2, icon_key, item_key, kcal_per_100g,
  ...
  const meals: NutritionMeal[] = rawMeals.map((m: any) => ({
    ...
    entries: (m.nutrition_entries ?? []).map((entry: any) => {
      const food = entry.food_items
      const localizedName = lang === 'fr'
        ? food?.name_fr
        : food?.food_item_translations?.find((translation: any) => translation.lang === lang)?.name ?? food?.name_fr
      return {
        ...entry,
        food_items: food ? { ...food, name: localizedName } : food,
      }
    }),
  ...
  // Rest-day names were historically persisted in French; use the UI label so
  // a Spanish client never receives the legacy "Jour repos" value.
  const dayTypeLabel = isTrainingDay
    ? String((protocolDay as Record<string, unknown> | null)?.name ?? ct(lang, 'smart.workout.rest'))
    : ct(lang, 'smart.workout.rest')
  const dayTypeBadge = (
    <span
